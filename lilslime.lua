...
require("helpfuldog")
require("hiteffect")
LilSlime = Enemy.new(0, 0, 16, 16)
LilSlime.__index = LilSlime
...
function LilSlime.new(x, y, player)
	local self = setmetatable({}, LilSlime)
	self.x = x
	self.y = y
	self.player = player
	self.v = {x=0.5, y=0}
	...
	self.type = "enemy"
	self.image = love.graphics.newImage("assets/img/lilslime.png")
...
function LilSlime:updateAnimation(dt)

	self.currentAnim:update(dt)
	self:flip(self.v.x<0)
end
function LilSlime:takeDamage(e, amt)
...
function LilSlime:flip(reverse)
	if (reverse) then
